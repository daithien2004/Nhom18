...
export const cancelFriend = async (currentUserId, targetUserId) => {
  ...
  // Tìm conversation và cập nhật status thành pending
  const conversation = await conversationRepo.findConversationBetweenUsers(
    currentUserId,
    targetUserId
  );

  if (conversation) {
    await conversationRepo.updateStatus(conversation._id, 'pending');
  }
